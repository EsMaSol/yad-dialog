              ...
              case YAD_FIELD_TEXT:
                {
                  ...
                  gtk_table_attach (GTK_TABLE (w), b, 0 + col * 2, 2 + col * 2, row, row + 1, GTK_EXPAND | GTK_FILL,
      ...
      switch (fld->type)
        {
          ...
          case YAD_FIELD_DATE:
            g_printf ("%s%s",
                      gtk_entry_get_text (GTK_ENTRY (g_slist_nth_data (fields, i))), options.common_data.separator);
            break;
          case YAD_FIELD_NUM:
            g_printf ("%f%s",
          ...
          case YAD_FIELD_CHECK:
            g_printf ("%s%s",
                      gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (g_slist_nth_data (fields, i))) ? "TRUE" :
